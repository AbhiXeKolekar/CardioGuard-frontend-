...
export const SimulationProvider = ({ children }) => {
  ...
  const [vitals, setVitals] = useState({
    hr: 75, spo2: 98.0, fatigue: { overall: 0, cardio: 0, muscle: 0, leg: 0 }
  });
  ...
  useEffect(() => { addLog(`Phase shifted to: ${phase}`, 'info'); }, [phase]);
    ...
    if (isRunning) {
      interval = setInterval(() => {
        ...
        setVitals((prev) => {
          ...
          let phaseMult = phase === 'Warm-up' ? 0.8 : phase === 'Active' ? 1.0 : phase === 'Peak' ? 1.3 : 0.6;
          ...
          if (currentStatus !== prevStatusRef.current) {
            if (currentStatus === 'danger') addLog('CRITICAL RISK: Vital signs exceeding safe thresholds. Stop activity immediately.', 'danger');
            else if (currentStatus === 'warning') addLog('HIGH STRAIN DETECTED: Consider reducing intensity. Hydrate and monitor breathing.', 'warning');
            else addLog('Vitals stabilized. System nominal. Safe to continue.', 'success');
          ...
          const newVitals = {
            ...
            fatigue: { overall: Number(newOverall.toFixed(1)), cardio: Number(newCardio.toFixed(1)), muscle: Number(newMuscle.toFixed(1)), leg: Number(newLeg.toFixed(1)) }
          };

          setHistory(h => [...h, { time: new Date().toLocaleTimeString([], { hour12: false, hour: '2-digit', minute:'2-digit', second:'2-digit' }), ...newVitals, ...newVitals.fatigue }].slice(-60));
  ...
  const resetSimulation = () => {
    setIsRunning(false);
    setElapsedTime(0);
    setVitals({ hr: 75, spo2: 98.0, fatigue: { overall: 0, cardio: 0, muscle: 0, leg: 0 } });
    setHistory([]);
    setLogs([]);
  ...
  return (
    <SimulationContext.Provider value={{
      ...
      scenario, setScenario, intensity, setIntensity, vitals, history, elapsedTime, logs, resetSimulation
    }}>
      {children}
